...
public class ControlXadrez {
     ...
     * @throws
     * ifes.poo1.xadrez.util.exception.CapturaInvalidaPecaPropriaException
     * @throws
     * ifes.poo1.xadrez.util.exception.CapturaInvalidaPecaInexistenteException
...
//                    }
    ...
    /**
     * Cria e retorna uma String contendo os dados do histórico de partidas,
    ...
    public List<Checkpoint> exibirJogosSalvos() {
        try {
            checkpoints = checkpointDAO.findAll();
        } catch (SQLException | ClassNotFoundException ex) {
            Logger.getLogger(ControlXadrez.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void carregarJogo(Checkpoint jogoCarregado) throws JogoInexistenteExcpetion {
    ...
    private void mudarVezJogador() {
        if (jogo.getVez().getPontuacao() >= 999) {
            ...
            vencedor = jogo.getVez();
                        //mudarVezJogador();
            //perdedor = jogo.getVez();
            if (vencedor == jogo.getBranco()) {
                perdedor = jogo.getPreto();
            } else {
                perdedor = jogo.getBranco();
            }
            finalizarJogo(vencedor, perdedor);
        }
