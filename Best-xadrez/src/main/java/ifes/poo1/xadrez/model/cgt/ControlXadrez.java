...
//                controlarComandoRecebido();
        ...
        /**
         * Controla a realização de roque menor pelos jogadores, tratando possíveis erros se assim
         * for necessário.
         *
        ...
        /**
         * Controla a realização de roque maior pelos jogadores, tratando possíveis erros se assim
         * for necessário.
         *
         * @throws ifes.poo1.xadrez.util.exception.RoqueInvalidoReiMovimentadoException
         * @throws ifes.poo1.xadrez.util.exception.RoqueInvalidoTorreMovimentadaException
         ...
         *
         * @throws RoqueInvalidoReiMovimentadoException - se o rei já tiver se movimentado
         * anteriormente.
         * @throws RoqueInvalidoTorreMovimentadaException - se a torre já tiver se movimentado
         * anteriormente.
         * @throws RoqueInvalidoCaminhoBloqueadoException - se o caminho do rei estiver bloqueado
         * por outras peças..
         * @throws RoqueInvalidoReiAmeacadoException - se alguma das posições pelas quais o rei vai
         * passar estiver ameaçada.
         */
         ...
         * @param jogada - a jogada realizada pelo jogador.
         */
        private void controlarCapturaPeca(Jogada jogada) throws CasaVaziaException, PecaAlheiaException, CaminhoBloqueadoException, MovimentoInvalidoException, CapturaInvalidaPecaInexistenteException, CapturaInvalidaPecaPropriaException {
        ...
        /**
         * Finaliza o jogo, adicionando os dados tanto do jogo quanto dos jogadores ao histórico que
         * será exibido nos Dados das Partidas.
         *
...
//        controlarMenuInicial();
        ...
        /**
         * Valida para quais posições o rei pode se movimentar/capturar, excluindo a propria
         * posicao.
         *
        ...
        /**
         * Determina o que irá acontecer após o pedido de desistencia, dependendo da opção que os
         * jogadores decidirem.
         *
...
//                controlarComandoRecebido();
        ...
        /**
         * Determina o que irá acontecer após o pedido de empate, dependendo da opção que os
         * jogadores decidirem.
         *
...
//                controlarComandoRecebido();
        ...
        /**
         * Verifica se um jogador se encontra na lista de jogadores do histórico, pelo nome do
         * jogador.
         *
        ...
        /**
         * Controlar o aumento de empates para os jogadores. Se o jogador já estiver na lista de
         * jogadores do histórico, apenas aumenta a quantidade de empates. Caso contrário, cria-se
         * um novo jogador para o histórico.
         *
         */
         ...
         * @throws MovimentoInvalidoException - se a peça não conseguir fazer o movimento
         * especificado.
         * @throws CaminhoBloqueadoException - se a peça conseguir realizar o movimento, mas tiver o
         * caminho obstruído.
                ...
                if (!peca.getCor().equals(jogo.getVez().getCor())) {
                        ...
                        if (peca.mover(posicaoFinal)) {
                                ...
                                if (jogo.getTabuleiro().verificaCaminhoMovimento(posicaoInicial, posicaoFinal)) {
                                        ...
                                        GUIControl.getInstanceOf().mensagemMoverPeca(posicaoInicial, posicaoFinal, jogo.getVez());
                                        ultimaJogada = peca;
                                } else {
         ...
         * @throws PecaAlheiaException - se a peça que estiver na posicaoInicial não for do jogador
         * da vez.
         * @throws CaminhoBloqueadoException - se a peça que estiver na posicaoInicial tiver o
         * caminho bloqueado para fazer a captura.
         * @throws MovimentoInvalidoException - se a peça que estiver na posicaoInicial não souber
         * fazer o movimento pedido.
         * @throws CapturaInvalidaPecaInexistenteException - se não tiver nenhuma peça em
         * posicaoFinal.
         * @throws CapturaInvalidaPecaPropriaException - se a peça que estiver em posicaoFinal for
         * do próprio jogador da vez.
         */
...
//                controlarJogoMultiplayer();
        ...
        /**
         * Controla as funcionalidades que ocorrem quando o jogador escolher a opção sair, no menu
         * inicial. No caso, os dados das partidas e dos jogadores são salvos em arquivo.
         *
         */
...
//            realizarJogadaSingleplayer();
        ...
        private Jogada determinarJogada(Posicao posicaoInicial, Posicao posicaoFinal) {
                Jogada jogada = new Jogada();
                if (this.getJogo().getTabuleiro().getCasas(posicaoFinal) == null) {
                        jogada.setTipoJogada(TipoJogada.MOVIMENTO);
                } else {
                        jogada.setTipoJogada(TipoJogada.CAPTURA);
                }
                jogada.setPosicaoInicial(posicaoInicial);
                jogada.setPosicaoFinal(posicaoFinal);
                return jogada;
        }
         ...
         *
         */
        public void realizarJogadaSingleplayer(Posicao posicaoInicial, Posicao posicaoFinal) throws PecaAlheiaException, MovimentoInvalidoException, CaminhoBloqueadoException, CasaVaziaException, CapturaInvalidaPecaInexistenteException, CapturaInvalidaPecaPropriaException {

//        controladorTela.mostrarTabuleiro(jogo.getTabuleiro());
                Jogada jogada = determinarJogada(posicaoInicial, posicaoFinal);

                controlarComandoRecebido(jogada);
                mudarVezJogador();
...
//    }
         ...
         *
         */
        public void realizarJogadaMultiplayer(Posicao posicaoInicial, Posicao posicaoFinal) throws PecaAlheiaException, MovimentoInvalidoException, CaminhoBloqueadoException, CasaVaziaException, CapturaInvalidaPecaInexistenteException, CapturaInvalidaPecaPropriaException {
